    ...
    public partial class FormCuenta : Form
    {
        ...
        private void BtnDesactivar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Debe Seleccionar una Cuenta Antes de Desactivar.", "ATENCION", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                }
                else
                {
                    ...
                    if (MessageBox.Show("¿Esta Seguro de Desactivar Cuenta " + _rut_usuario + "  Para la Empresa " + auxEmpresa.Nombre + "?", "CONFIRMAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        auxServiceCuenta.ActualizarCuentaSinEntidad_Escritorio(auxCuenta.Rut_usuario, auxCuenta.Rut_empresa, 0, auxCuenta.Id_rol, auxCuenta.Correo);
                        btnActivar.Visible = false;
                        btnDesactivar.Visible = false;
                        //Vaciar variables
                        _rut_usuario = null;
